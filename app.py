...
# --- TensorFlow for TFLite model loading and prediction ---
# We will explicitly use tensorflow-cpu on Render for better compatibility
import tensorflow as tf

# --- Flask App Initialization ---
app = Flask(__name__)
# A temporary secret key. For production, set FLASK_SECRET_KEY as an environment variable on Render.
app.secret_key = os.getenv("FLASK_SECRET_KEY", "your_strong_random_secret_key_here_for_testing") 
...
# --- GLOBAL MODEL LOADING (Using TFLite - This is crucial for performance) ---
interpreter = None
input_details = None
output_details = None
try:
    # Path to your quantized TFLite model (ensure it's in the 'models' directory)
    tflite_model_path = os.path.join(os.path.dirname(__file__), 'models', 'plant_disease_model_quantized.tflite')
    ...
    print(f"TFLite Disease prediction model loaded successfully from {tflite_model_path}")
    print(f"TFLite Input details: {input_details}")
    print(f"TFLite Output details: {output_details}")

except Exception as e:
    print(f"Error loading TFLite disease prediction model: {e}")
    interpreter = None # Set to None if loading fails

# --- Disease Solutions and Labels (Provided by you) ---
disease_solutions = {
    'Potato___healthy': {
...
# --- Main Disease Prediction Route ---
@app.route('/', methods=['GET', 'POST']) # Change route to '/' for the main page of this new app
@app.route('/disease-predict', methods=['GET', 'POST']) # Keep for compatibility, but primary will be /
def disease_predict():
    image_url = None
    ...
    if request.method == 'POST' and 'imageUpload' in request.files:
        ...
        if file.filename == '':
            ...
            try:
                ...
                if interpreter: # Use interpreter for TFLite model
                    # Get input image dimensions from the TFLite model's input details
                    input_shape = input_details[0]['shape']
                    target_height, target_width = input_shape[1], input_shape[2] # Assuming (batch, H, W, C)
                    
                    img_resized = img.resize((target_width, target_height))
                    
                    input_dtype = input_details[0]['dtype']
                    img_array = np.array(img_resized, dtype=input_dtype)

                    # Add batch dimension
                    img_array = np.expand_dims(img_array, axis=0)
                    
                    # Normalize if the TFLite model expects values between 0-1 (common for float32)
                    ...
                    # Assuming output is a batch of predictions, get the first one
                    ...
                    if 0 <= predicted_class_index < len(disease_labels):
                        ...
                        confidence_score = predictions_single[predicted_class_index] * 100
                    ...
                    error_message = "Disease prediction model not loaded or failed to initialize."
...
if __name__ == '__main__':
    # When running locally, Flask development server listens on port 5000 by default.
    # For Render, the Procfile will tell Gunicorn to listen on the PORT environment variable.
